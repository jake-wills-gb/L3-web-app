				...
				if($concours != NULL){
					foreach($concours as $ligne){
						if(!isset($traite[$ligne['CCR_concours_id']])){
							...
							echo '<td><button type="button"'; if($ligne['phase']!='finale'){echo 'disabled';}else{echo '';} echo '>Voir les candidats</button></td>';
							echo '<td><button type="button"'; if($ligne['phase']!='termine'){echo 'disabled';}else{echo '';} echo '>Voir palmares</button></td>';
						}
						echo '</tr>';
					}
				}
